...
本地同名资源目录中命中的项目内图片，只要已有可用 SpriteFrame，三/九宫与普通 PNG 一样直接引用原 UUID，沿用本地切片边距。不会重新下载、最小化、另存同名副本或修改素材库 Meta；强制刷新也不会覆盖这一优先级。没有本地候选且未强制刷新时，导入目录已有的同名九宫资源同样直接复用。

本地 SpriteFrame 没有九宫边距时仍复用原资源，提示按普通 Sprite 显示并请用户配置原资源；项目内图片尚未生成可用 SpriteFrame 时提示错误，不偷偷复制替代。项目外素材没有可引用的 Cocos UUID，仍需导入到项目。匹配使用 Figma 原始节点名，不使用插件内的改名。新生成的九宫资源才执行下述最小化流程。
...
- 下载缓存仍保存完整原图；项目内本地同名 SpriteFrame 优先直接引用并跳过最小化。只有没有可复用资源时才处理输出图片（项目外素材仍需导入），不批量改写素材库。普通 PNG、Tiled 和原图直引路径不参与最小化。
