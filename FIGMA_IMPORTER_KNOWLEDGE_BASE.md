...
### 2026-09-12 · V2 导入结果检查

- 基于 `v2-MCP` 开发，保留该分支既有 MCP、Mask、字号/行高和切片规则。完成导入后面板自动打开结果检查；MCP 导入也生成报告并通知已打开的面板，MCP 简要结果增加 `reviewAvailable`。
- `ImportReviewRecorder` 在资源写入前记录文件是否已存在及覆盖前缩略图，按最终资产 URL 合并资源行、按真实 Scene 引用列出消费者。“下载”“缓存命中”不能直接判定为新文件；覆盖已有/本地复用资源不允许删除。
- Scene 在构建前捕获节点快照，构建与保存后捕获结果，按 Prefab fileId（普通场景为 UUID）对齐；报告显示新增、删除、名称、父级、顺序、显隐、几何和主要组件属性变化，并标识插件受管组件。Figma 原树与最终映射作为另一视图保留。
- 面板勾选只存于本地待确认集合。主进程收到“确定修改”后校验报告编号、允许删除集合、文件/Meta 哈希、编辑目标、目标节点树与其他引用；Scene 只清空受管 Sprite.spriteFrame，保留节点、组件、尺寸和 Tiled/Sliced 属性。通过 AssetDB 删除图片及其 Meta；不得直接递归删除目录。
- 删除前保存原图片、Meta、目标文件与 manifest 到项目 `temp/figma-importer/review-backups/<id>`。目标保存失败则尝试恢复引用并保留文件；个别文件删除失败则保留文件、显示明确提示。确认过程中核查已保存目标的 UUID 引用，排除 AssetDB 依赖索引更新延迟造成的误判。
- 已知限制：报告只保留当前插件会话最新一次结果；新场景须先保存为资源再重新导入以启用删除。覆盖前预览有 24 MB 总预算及单图 16 MB 读取上限，实际图片预览上限 32 MB；Figma 来源按需读取当前版本，复杂隐藏节点预览可能透明。组件对比不是任意用户脚本的完整字段审计。项目临时目录被清理后备份不再保留。Creator 3.8.7 真机 AssetDB 删除与保存链路仍需实际验收。
...
### 1.2 Git 引用路径与 SourceTree 兼容约束

**维护要求（2026-09-09）：后续开发必须保持 SourceTree 能正确读取本地及远程分支；Git 命令行正常不等于 SourceTree 正常。**

- 手动创建的目录、临时文件及 Git 引用使用短名称、浅层级；完整绝对路径以不超过 240 个字符为保守预算（不是工具兼容性保证），不要叠加多层完整哈希、时间戳和 UUID。计算长度时包括仓库根路径与 Git 公共目录；worktree 应用 `git rev-parse --path-format=absolute --git-common-dir` 定位实际引用目录。
- 已复现案例：Windows SourceTree 3.4.32 自带 LibGit2Sharp 遍历 `refs/codex/turn-diffs/checkpoints/…` 时，262 字符的路径使引用枚举提前结束，表现为历史图有分支标签、左侧分支列表却为空；同一份引用放到短路径后正常。对照仓库 PSD_Figma 的检查点路径为 257 字符，当时正常，但余量不足，不能作为安全标准。
- Codex 自动生成的检查点路径不由插件业务代码控制；本约定不能保证应用自动遵守短路径预算。不得擅自截断、重命名或删除检查点引用，也不得宣称改了本文档就永久解决。优先为新工作副本选择短根目录；迁移现有仓库需另行确认，并更新相关工具的绝对路径。
- 出现分支列表异常或发现超长引用时，先停止并发 Git 写入，备份实际 Git 公共目录中的 `refs` 和已有的 `packed-refs` 到引用目录之外。记录 `git for-each-ref --format="%(refname) %(objectname)"`、`git rev-parse HEAD` 和 `git status --porcelain` 的结果，然后在目标仓库执行：

```powershell
git -c core.longpaths=true pack-refs --all
```

- 这是保留引用名及目标对象的存储整理，不是删除检查点、合并分支或推送。不要手工编辑 `packed-refs`，也不要用删除 `.git/refs/codex`、`reset --hard` 或强制推送替代。执行前确认目标仓库与备份，命令失败立即停止。
- 整理后逐项核对全部引用名/对象 ID、HEAD 和工作区状态不变，再重新打开 SourceTree 仓库标签页，确认本地及远程分支均可列出；不能只以 `git branch` 成功验收。必要时使用当前安装的 LibGit2Sharp 作只读枚举复测。
- `core.longpaths=true` 仅用于上述 Git 命令，不能证明 SourceTree 内置库已兼容长路径。后续自动生成超长检查点仍可能复发；开发交接时检查路径风险及分支可读性，发现复发按上述流程处理。本次未安装自动清理、定时任务或 Git hook。
...
- 第一个命中目录优先；
- 同目录重复命中按确定性排序取第一个；
- 本地资源目录不等于输出目录。命中当前项目 `assets` 内已有可用 SpriteFrame 的普通 PNG 或三/九宫图片时，优先直接引用其 UUID，沿用本地边距，不下载、不最小化、不复制，也不修改原 Meta；`refreshAssets` 不覆盖显式本地目录的命中。
- 三/九宫本地 SpriteFrame 没有边距时仍直接引用并提示暂按普通 Sprite 显示；项目内图片缺少可用 SpriteFrame 时停止并提示，不另建同名副本。Figma 切片几何识别只用于生成新资源，不作为引用本地九宫的前提。
- 未命中本地目录且未启用刷新时，导入目录内已有同名九宫资源也直接复用。项目外素材没有 Cocos UUID，仍作为输入素材导入项目。
...
### 冻结协议逐字节回归在 Windows 失败（1.0.56 修复）

全局 `core.autocrlf=true` 曾将 manifest 锁定的全部 12 个 LF 文件检出为 CRLF，导致第一项 canonical-json.ts 哈希校验失败。逐一验证发现仅 CRLF→LF 即可恢复原始 12 个 SHA-256；新增精确路径 `.gitattributes` 强制这些文件 `text eol=lf`，并将当前副本按原始字节恢复。逐字节门禁及 manifest 不变，不在测试中忽略换行或接受多套哈希。新增测试保证每个 manifest 文件有明确的 LF 规则；后续新增锁定文件须同步维护规则。

不修改用户全局 Git 配置，也不影响其他源文件或项目资产。下方历史版本记录中的“唯一失败”保留为当时事实，本问题自 1.0.56 起已修复。

### 全新导入也提示“保存后节点或组件内容已变化”（1.0.55）

该检查比较同一次导入保存前后的快照，与旧文件是否删除无关。旧 refreshReviewAfter 将完整 nodes JSON（仅排除运行时节点 UUID）直接比较，文字/布局延后更新宽高等几何差异也导致只读。1.0.55 改为按稳定节点/组件标识逐项核对；仅导入收尾允许几何变化并报告前五项，采用保存后快照建立新的严格清理基线。非几何属性、结构、管理归属和资源引用变更仍拒绝；等价 UUID 编码、对象/属性枚举顺序不应触发误报，真实节点顺序仍保护。

原始日志未保存字段差异，不能反推 02:24:14 那次具体变化；后续阻断会给出最多五项具体节点/组件/字段差异及前后值。仍无法证明的变化继续只读，不按名称猜测替换对象、不忽略组件增删、不关闭引用保护。正常几何同步在报告提示中可见，不主动修改 Prefab、资产或引擎设置。真实 Creator 重导入尚待验收；报告生成后的手动几何修改依然会使删除前严格校验失效。

### 资源对比列表点击后回到顶部（1.0.50 修复）

旧回调 `this.selected = asset.id; this.render()` 会清空并重建 `.review-content`，导致内层 `.review-resource-list` 的 scrollTop 和 DOM 焦点丢失。点击改为原位更新各行 selected/aria-pressed，并只替换右侧详情；每次切换递增预览序号，避免旧请求覆盖新资源。重复点击当前资源直接返回。筛选、搜索、翻页仍允许列表重建。

### 资源定位累积多选（1.0.49 修复）

Creator 的 `Editor.Selection.select('asset', uuid)` 会追加资源选择。导入报告的定位按钮现在先 `clear('asset')` 再 `select`，每次只保留目标资源；不清除场景节点选择，也不触发资源删除/报告确认。

### 导入对比触发 `Since v3.6…mask…` 警告（1.0.48 修复）

原因是旧快照代码对所有组件试读 `color`，Mask 兼容接口每次读取均会调用 `warnID(9007)`；一次存在性判断加 RGBA 四次读取导致“一条＋重复四次”。1.0.48 将 Mask 从通用属性采集分支分离，只记录当前支持的遮罩属性与实际 Sprite 的资源引用。颜色采集只允许 Sprite/Label/RichText/Graphics，并缓存 getter 返回值；不关闭控制台警告、不删除或修改 Mask。重新加载插件后需重新导入生成快照；先前的控制台记录需自行清空。

此修复不代表已解决 Tiled 缓冲区超限或报告编辑目标校验，它们仍为独立待处理事项。
...
`1.0.45` 代码级回归（2026-09-03）：`Label.fontSize` 与 `RichText.fontSize` 不再乘导入倍率，并按 Figma 面板精度最多保留两位小数；导入倍率为 `2` 时，内部值 `17.8873233795166` 写入 Cocos `17.89`，Figma `24.5` 仍写入 `24.5`。Scene 定向测试 `67/67`、TypeScript 构建及分发门禁通过；完整 `npm test` 为 `195/196`，唯一失败仍是未修改的 Round-trip 冻结文件 Windows CRLF 原始字节哈希差异。
...
### 2026-09-13 · `1.0.56` · 修复冻结协议 Windows 换行回归

- 确认 12 个 manifest 锁定文件均仅因 CRLF 与原哈希不一致；恢复 LF 后全部匹配原始 SHA-256，不修改协议、manifest 哈希或原有逐字节测试。
- 新增 `.gitattributes` 精确路径规则固定 12 个文件为 LF；新增规则覆盖回归，避免以后新增冻结文件遗漏跨平台保护。不修改全局 Git 设置、其他工作区文件或导入逻辑。
- 验证：完整 `npm test` 353/353 全部通过，Cocos/MCP 构建与分发检查通过；12 个冻结文件分别在 Git autocrlf=true/false/input 的检出过滤下均匹配原始哈希（36 项检查）。`git diff --check` 通过，冻结文件相对 Git 原始内容无差异。
- 未提交或推送。

### 2026-09-13 · `1.0.55` · 保存收尾分级校验与差异诊断

- compareSavedReviewScene 按稳定身份核对节点结构和全部已记录组件属性；资源 UUID 等价表示规范化后比较，子资源后缀仍区分；拒绝空/重复标识、组件增删/替换、归属变化、资源丢失或替换及无效几何。
- 仅收尾阶段接纳几何更新，返回保存后实际快照及 saveAdjustments（计数、最多五项详情），在导入提示呈现。refreshReviewAfter 失败列出具体节点/组件/字段差异；不改变节点、组件、图片、Tiled 或容量设置。
- sessionFor 的严格指纹及项目/场景引用扫描、文件哈希、保存、备份、删除规则不变；报告生成后发生几何改动仍不允许按过期报告清理。
- 报告消费者/三九宫关联识别、清理槽位及场景外部引用扫描使用同样的 UUID 规范化，避免允许等价编码后漏掉真实引用或无法清空对应槽位。
- 新增保存几何接纳、确认后过期保护、外部引用保护、等价 UUID、真实引用变化、结构/属性风险、差异限长、完整报告回填测试；已有重载测试增加保存后几何变化与真实对象清理尺寸保留断言。
- 验证：导入检查定向测试 39/39，完整回归 351/352；唯一失败仍为冻结协议 canonical-json.ts 的既有 Windows CRLF 字节哈希校验。Cocos/MCP 构建和分发检查、真实编译面板无头浏览器检查及 diff 空白检查通过。
- 真实 Creator 重新导入尚待用户验收；旧日志无法确认具体字段，不声称已经复现该次快照。未提交或推送。

### 2026-09-13 · `1.0.54` · Tiled 图片像素承载实际尺寸，节点不再补偿缩放

- 新增 tiled-png：PNG 面积缩小/周期边界双线性放大，透明度加权避免透明 RGB 污染，保留 PNG 色彩元数据；非 PNG 通过 Creator Electron 解码。源/目标大小、倍率、位深和动画格式有边界检查，失败明确报错而不悄悄退回节点缩放。
- processTiled 在原图下载/缓存之后生成实际尺寸 PNG，按 requestedScale / sourceRenderScale 处理 IMAGE 与 PATTERN，避免已按倍率导出的 PATTERN 再缩一次；新绑定 tileScale=1。全局导入倍率仍参与图片尺寸，非整数像素取整并提示。
- 资源 key 增加 figma-visible-pixels-v1 和请求倍率，旧原图资产不被误复用，原下载缓存保持兼容且不写入缩小结果。普通 PNG、三/九宫和多填充/布尔节点原生 Tiled 排除条件保持不变。
- 现有 Scene 路径对新矩形资源直接挂载 TILED Sprite，Scale=1，无缩放辅助节点；旧 payload 的 tileScale 保持兼容。重导入迁移测试验证旧辅助节点移除、手工子节点保留、父节点位置和尺寸不变；椭圆遮罩继续保留且 Sprite Scale=1。
- 用当前真实 img_jade_panelbg2__tile_4427ae0708.png 只读试算，26×28→13×14，输出 PNG 194 字节，原资产字节未变。没有替用户修改当前 Prefab、手工节点、Figma 文件或全局缓冲设置，未提交/推送。
- 同纹理密度的格数不变，本次不声称解决 StaticVBAccessor 超限；需重启 Creator 后重新导入进行真实视觉验收。
- 验证：新增 9 项资源测试和 2 项 Scene 测试通过，椭圆旧测试补充 Scale=1 断言；完整回归 341/342，唯一失败仍为原有冻结协议文件 Windows CRLF 哈希问题。Cocos/MCP 构建、分发检查和 `git diff --check` 通过。

### 2026-09-13 · `1.0.53` · 描边宽度保留两位小数

- Label.outlineWidth 与 Graphics.lineWidth 统一在倍率和最小宽度计算后取两位小数；保留原有最小值，不改变 Figma 原始数据、图片内描边、颜色、Tiled 或节点坐标。不直接写入当前项目 Prefab。
- 新增文字/图形回归，覆盖舍入、进位、倍率后舍入及原有最小值。
- 验证：Cocos/MCP 构建及分发检查通过；完整测试 330/331，唯一失败仍为既有冻结协议文件的 Windows CRLF 哈希差异；`git diff --check` 通过。未提交或推送。
- 排查 00:48:17 的 JadeView 导入：平铺资源 count=0，当前 Prefab 为 SIMPLE 51、SLICED 2、TILED 0，已无旧 img_jade_panelbg2 辅助树。尚需具体 Figma 目标节点才能判断预期 Tiled 为何未命中，未擅自将普通 Sprite 改为平铺。
- 00:48:31 的“导入编辑目标已切换”仍来自旧 refreshReviewAfter 实现（旧错误文字和堆栈），需完整重启 Creator 加载 1.0.52 起的修复后复验。该轮导入日志没有新增 StaticVBAccessor 错误，最近一次在导入前的 00:44:44。

### 2026-09-13 · `1.0.52` · 导入报告校验与弹窗兜底

- 修复 attachReview 将 refreshReviewAfter 的异常连同整个报告吞掉的问题。报告独立收尾，短暂目标不稳定最多尝试三次；失败仍缓存并展示只读报告。完整报告分析失败也降级为无外部读写依赖的基础报告。
- Prefab 注册使用已核实的目标 UUID；仅在目标 UUID/模式、根 fileId、节点结构和记录组件属性一致时重新绑定保存后重载的对象。原资源清理的指纹、外部引用、备份和保存校验继续保留。
- readOnlyReason 同时控制界面勾选、确定按钮和后台确认接口；只读节点对比明确标注为历史快照。自动打开面板并发送报告，面板就绪补取未确认报告，相同报告重复通知不重置勾选。
- 不修改图片导出、坐标体系、九宫压缩、Tiled 或项目设置；未提交/推送。更新后需要完整重启 Creator，实际项目导入验收由用户执行。
- 验证：报告定向测试 29/29，真实编译面板浏览器检查通过（只读弹窗/禁止确认、重复通知保留勾选、原滚动和定位回归）；完整测试 328/329，唯一失败仍为既有冻结协议文件 CRLF 哈希差异。Cocos/MCP 构建与分发检查、`git diff --check` 通过。

### 2026-09-13 · `1.0.51` · 来源页区分三/九宫合并

- ImportReview.sourceTree 新增可选 sliceMerge（模式、Figma 父 id、实际 Cocos 目标名）；主进程根据记录的 sliced 资源、实际映射节点的 Sprite 类型/资源引用及分析通过的 3/9 宫几何生成标记。不凭名字或单纯缺少映射猜测。
- 无独立目标的切片子层显示“九宫合并 → 父资源名”或“三宫合并 → 父资源名”，并解释不单独创建节点。独立目标优先；普通 PNG、Tiled、失败降级、未导入父节点和直接隐藏的非参与子层不误标。
- 只修改报告模型与展示，不修改切片分割线、PNG 最小化、节点合并或资源清理行为。旧报告需重新导入生成标记；没有自动提交或推送。
- 新增五项模型回归（九宫、横/纵三宫、非九宫排除、独立/隐藏子层），报告定向测试 23/23 通过；浏览器脚本新增三/九宫说明和普通未导入提示并存的校验。
- 验证：浏览器检查、构建、Cocos/MCP 分发和 `git diff --check` 通过；完整测试 322/323，唯一失败仍为既有 Round-trip 冻结文件换行符哈希问题。Creator 内重新导入验收尚待执行。

### 2026-09-13 · `1.0.50` · 修复资源对比列表滚动复位

- 不再在选择资源时调用整页 render，保留左栏 DOM、scrollTop、勾选和焦点，只更新右侧详情及选中状态；增加 aria-pressed 状态，同一资源重复点击不重新请求预览。
- 浏览器回归构造 75 个资源，分别在 920px、420px 宽度下滚动至下方行，验证连续切换后 DOM 引用/滚动值不变、详情正确切换、仅一项高亮、勾选与键盘焦点保留，并继续验证单选定位与确认清理流程。
- 验证：浏览器脚本通过；`npm test` 构建、Cocos/MCP 分发检查通过，完整测试 317/318，唯一失败仍为既有 Round-trip 冻结文件换行符哈希问题；`git diff --check` 通过。尚未在 Creator 内手动操作验收。
- 沿用尚未提交的 Mask 警告和单选定位修复；没有自动提交或推送。搜索/筛选/分页策略及资源导入、删除确认逻辑未修改。

### 2026-09-13 · `1.0.49` · 资源面板定位改为单选

- “在资源面板中定位”在选择目标前只清空 asset 类别的选择，避免 Creator 将连续定位解释为多选。未改全局选择逻辑或导入完成时的资源选择。
- 浏览器回归覆盖已有多选→单选、连续定位新建/引用已有/覆盖已有资源、重复点击同图、场景选择保留、报告勾选不变及已删除资源无定位按钮。
- 验证：`node scripts/verify-import-review-ui.js` 通过；`npm test` 构建和 Cocos/MCP 分发检查通过，测试 317/318，唯一失败仍为既有 Round-trip 冻结文件换行符哈希问题；`git diff --check` 通过。浏览器使用真实编译面板及模拟 Creator Selection，未替用户在 Creator 中执行导入或清理。
- 沿用同一 V2 工作区中尚未提交的 1.0.48 Mask 警告修复；本次没有自动提交或推送。

### 2026-09-13 · `1.0.48` · 修复报告采集 Mask 旧属性警告

- 影响范围仅 `source/import-review-scene.ts` 的只读快照采集：Mask 使用独立属性白名单，跳过旧 color/spriteFrame 及渲染兼容属性；Sprite 类型的 subComp 仍能提供贴图引用，矩形/Graphics 遮罩无须依赖 Sprite。
- 颜色只在 Sprite/Label/RichText/Graphics 上采集，各 getter 每次快照只读一次；Graphics 同时记录 fillColor/strokeColor。不再试读自定义非渲染组件的 color。
- 不改遮罩设置/效果、不修改节点位置尺寸、不改变九宫最小化或 Tiled，不修改报告目标校验。
- 回归新增四项：Mask 旧 getter 访问即抛异常且 capture/register/refresh 均不触发；无 Sprite 遮罩；渲染颜色只读一次且变化仍可比较；自定义 color 不被读取。定向报告测试 18/18 通过；`npm test` 完整测试 317/318，唯一失败仍为既有 Round-trip 冻结文件 Windows CRLF 原始字节哈希差异；构建、Cocos/MCP 分发检查及 `git diff --check` 通过。
- Creator 实机重新导入尚待用户验证；更新后重新加载插件并重新生成报告，旧日志不会自动清除。

### 2026-09-12 · `1.0.47` · V2 恢复三/九宫图片最小化

- 历史来源：`main` 的 `a42fcc7`（2026-08-21）包含 `compactSlicePng`，当前 V2 在该提交前分叉，因此重新移植，未切换分支或合并 main 的其他功能。
- 实现：`source/importer/sliced-png.ts` 以逻辑单位边距及导出倍率计算像素边距，保留两逻辑像素中心带；横纵方向分别验证整个中心带的 RGBA 完全重复后再复制拼接，固定边角逐字节保留。不同像素、旋转/翻转、尺寸不符、高位深/特殊 PNG、超过 16×1024×1024 像素或 64 MiB 文件均保守跳过，支持只优化一轴。
- 色彩与缓存：保留 gAMA/cHRM/sRGB/iCCP/pHYs；不做有损重采样；缓存仍为最小化前原始 PNG，最小化在 `processRemote` 写资产前执行。普通 PNG、Tiled、原始 IMAGE/PATTERN 路径不变，节点几何与 `SLICED + CUSTOM` 继续由既有 Scene 逻辑负责。
- 失败保护：资源写入后必须精确回读四个边距和 `trimType=none`，不再仅以任意边距非零认定九宫成功；小图切片未确认时先恢复完整 PNG，再按原逻辑继续或降级 SIMPLE。基础 AssetDB 写入异常仍报告失败。
- 分发：pngjs 7.0.0 作为开发依赖，经 esbuild 打入 `dist/importer/sliced-png.js`，附 MIT 许可；使用者不需要安装运行依赖。更新源码后必须使用 `npm run build` 生成分发产物。
- 可观测性：诊断日志记录 status/reason、sourceWidth/sourceHeight、width/height、sourceBytes/bytes。跳过优化不是九宫失败，不混入“三/九宫降级”计数。
- 验证：新增水平/垂直/九宫、倍率、薄中心带、图案与 alpha 保护、单轴渐变、gamma、坏图/大图/尺寸保护、缓存重复执行、失败恢复和场景几何回归，共 24 项新增测试全部通过。`npm test` 构建、Cocos/MCP 分发检查通过，完整测试 313/314；唯一失败仍为既有 Round-trip 冻结文件 Windows CRLF 原始字节哈希差异。`git diff --check` 通过。
- 真实项目只读试算：`common_bg_tanchu_04` 488×472→26×26；`common_xian_03` 429×6→34×6；`img_bj_datc_a` 536×704→318×171；`img_zrjj_weibaomingd 1` 122×36→32×33。四张图片按原九宫边距还原尺寸后，逐像素 RGBA 差异均为 0。没有改写现有 PNG/Prefab；Creator 内重新导入与视觉验收尚待执行。之前的 Tiled 容量超限、报告 Mask 属性警告和报告目标校验问题不属于此次修改。

### 2026-09-12 · `1.0.46` · V2 导入收尾检查

- 新增资源对比弹窗、真实引用节点与来源说明、覆盖前/实际/Figma 图片预览、状态筛选和分页。
- 新资源勾选延迟到“确定修改”执行；增加节点树与文件变更校验、外部引用保护、备份及失败恢复，已有资源不可经此入口删除。
- 新增导入前后节点树高亮与组件主要属性、Figma 原始层级映射；支持面板重新查看和 MCP 导入完成通知。
- 添加清理服务/Scene 回归与真实浏览器面板交互脚本 `scripts/verify-import-review-ui.js`（通过 `CHROME_PATH` 可指定 Chrome/Edge）。清理专项测试 `14/14` 通过；构建、Cocos/MCP 分发和浏览器验证通过；完整回归 `289/290`，唯一失败仍为既有冻结文件 Windows CRLF 哈希差异，未修改冻结协议文件。

记录格式：`日期 · 版本/提交 · 变更 · 验证 · 影响/迁移说明`。

### 2026-09-09 · 仓库维护（不变更插件版本）

- 新增第 1.2 节 SourceTree 兼容约束：短路径预算、引用备份、标准打包流程与分支可读性验收；不擅自删除或改名 Codex 检查点。
- 前序排障确认 262 字符检查点路径导致内置 LibGit2Sharp 提前结束枚举；打包后恢复 3 个本地、6 个远程分支，全部 12 个引用、HEAD 和工作区状态保持不变。
- 本次仅更新知识库，未修改代码或构建产物，未运行业务测试/构建；提交前执行 `git diff --check`。自动检查点仍可能复发，不把文档约束当作应用层永久修复。

### 2026-09-03 · `1.0.45`

- 修改普通 Label 与 RichText 的字号写入标准：Cocos `fontSize` 不乘导入倍率，并将 Figma 内部高精度浮点值规范到属性面板最多两位小数的显示精度。
- 新增非 1 倍导入回归，确认内部 `17.8873233795166` 写为面板值 `17.89`，同时 Figma `24.5` 保持 Cocos `24.5`；其他节点几何和图片输出倍率逻辑保持不变。
- Scene 定向测试 `67/67`、TypeScript 构建及分发门禁通过；完整测试 `195/196`，唯一失败仍为未修改的 Round-trip 冻结文件 Windows CRLF 原始字节哈希差异。既有 Prefab 需重新导入后更新字号。
