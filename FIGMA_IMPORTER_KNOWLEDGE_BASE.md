...
│  ├─ cache.ts                     哈希键图片缓存
│  ├─ sliced-png.ts                三/九宫完整 PNG 的最小纹理重组
...
- 三宫/九宫候选按子节点名和几何覆盖关系识别。只有直接子层严格命名为 `Rectangle`、`Rectangle 1`…且几何完整组成横三段、竖三段或 3×3 网格时，智能模式自动启用最小化切片；仅几何上可能为网格的普通候选保持手动勾选，避免误裁剪。
...
| 三宫/九宫 | `Sprite.Type.SLICED` | 完整父层 PNG 先重组为最小纹理，SpriteFrame 写入重组后的边界元数据 |
...
- 严格 `Rectangle` 三/九格新增“最小纹理”导入：先请求父节点完整 PNG，再按 Figma 网格边界保留固定边缘与中心 `2` 个逻辑像素并重组 PNG；导入节点仍保持原 Figma 宽高，Cocos 使用 `Sprite.Type.SLICED` 拉伸中心区域。
- 边框写入基于重组后的实际像素，导入倍率为 `2` 时保留 `4` 个物理中心像素，保持与 1 倍导入一致的逻辑切片宽度；优先用与请求倍率匹配的 Figma 边界坐标映射，支持带渲染溢出的效果边界。
- 只有“命名严格 + 几何有效”的网格默认勾选；普通几何候选继续要求用户手动勾选。旋转、斜切、非 PNG 或像素边界无法可靠映射时回退为完整 PNG + 原切片边框并输出警告，不以错误裁剪换取体积。
- 资源覆盖为非切片 PNG 时会主动清除旧 SpriteFrame 九宫边框，避免复用同名文件后残留 `SLICED` 元数据。
- 新增三宫、九宫、高倍率、旋转安全回退及自动候选测试；`npm test` 共 `73` 项通过。
